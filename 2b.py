...
def polynomial_regression(input_data, nb_samples, nb_features, color):
    ...
    l2 = tf.multiply(0.01, tf.nn.l2_loss(w))
    loss = tf.add(tf.reduce_mean(tf.square(hyp - Y_col)), 0.01 * tf.nn.l2_loss(w))
        ...
        for epoch in range(nb_epochs):
            ...
            for sample in range(nb_samples):
                feed = {X: data['x'][sample].reshape((1, nb_features)),
                        Y: data['y'][sample]}
                _, curr_loss, l2_loss = sess.run([opt_op, loss, l2], feed_dict=feed)
...
def main():
    ...
    plt.plot(range(3, 4), losses, color='g')
    plt.xlabel("Degree")
    plt.ylabel("Loss")
    plt.show()
